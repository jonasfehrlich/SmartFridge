...
public class EinkaufslistenService implements Serializable {
    ...
    private static final String DATEI = "data/einkaufsliste.json";
    ...
    public void eintragHinzufuegen(Einkaufslisteneintrag eintrag) {
        if (!istBereitsAufListe(eintrag.getProduktname())){ 
    ...
    public ArrayList<Einkaufslisteneintrag> getEintraege() { 
    ...
    private boolean istBereitsAufListe(String name) {
        for (Einkaufslisteneintrag e : eintraege) {
            if (e.getProduktname().equalsIgnoreCase(name)) {
            	return true;
            	}
            }
        return false;
    ...
    public static EinkaufslistenService laden() {
        EinkaufslistenService s = Datenspeicher.laden(DATEI,EinkaufslistenService.class);
